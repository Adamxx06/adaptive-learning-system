...
export default function TopicView({ topicId, topics, onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    if (saved) setIsUnlocked(JSON.parse(saved).unlocked || false);
  ...
  useEffect(() => {
    if (score !== null) {
      ...
      localStorage.setItem(`topicProgress_${topicId}`, JSON.stringify({ score, unlocked }));
    }
  }, [score, topicId]);

  const handleSelectAnswer = (questionId: number, answer: string) => {
    if (score === null) setUserAnswers((prev) => ({ ...prev, [questionId]: answer }));
  };
  ...
  const getStudyLinks = () => {
    ...
    if (title.includes("javascript") || title.includes("js"))
      return [
        { name: "MDN JS Guide", url: "https://developer.mozilla.org/en-US/docs/Web/JavaScript" },
  ...
  return (
    <div className="topic-view py-5 px-3 px-md-5" style={{ background: "linear-gradient(180deg, #f8f9fa 0%, #e9f0f9 100%)", minHeight: "100vh" }}>
      <div className="container-lg">
        <h1 className="display-5 fw-bold mb-4 text-center text-primary">{topic.title}</h1>
        ...
        <section className="mb-5 p-4 bg-white rounded-4 shadow-sm border-start border-4 border-primary">
          ...
          <p style={{ whiteSpace: "pre-line", lineHeight: 1.8, fontSize: "1.05rem", color: "#222" }}>{topic.content}</p>
        </section>
        ...
        {topic.code_snippet && (
          <section className="mb-5">
            ...
            <pre className="p-4 rounded-4 text-light shadow-lg" style={{ background: "linear-gradient(135deg, #001f3f, #012a63, #023e8a)", overflowX: "auto", fontSize: "0.95rem", border: "none" }}>
          ...
          <section className="card border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <div className="card-body p-4 p-lg-5">
              <div className="d-flex flex-wrap justify-content-between align-items-center mb-4">
                <p className="lead fw-bold mb-0 text-dark">{score !== null ? `Your Score: ${score} / ${quiz.questions.length}` : `Attempt: ${attempts + 1}`}</p>
                {score !== null && (
                  <div>
                  ...
                  <div key={q.id} className="mb-4 p-4 border rounded-4 bg-white shadow-sm">
                    ...
                    <div className="d-grid gap-2">
                      {q.options.map((opt) => {
                        ...
                        if (showFeedback) {
                          if (opt === q.correct_answer) btnClass = "btn btn-success text-white text-start fw-bold shadow-sm";
                        ...
                        return (
                          <button key={opt} className={`${btnClass} text-wrap`} onClick={() => handleSelectAnswer(q.id, opt)} disabled={score !== null}>
              ...
              {/* Submit button */}
              {quiz.questions.length && score === null && (
                <div className="d-flex justify-content-end mt-4 pt-3 border-top">
                  <button onClick={handleSubmitQuiz} className="btn btn-primary btn-lg shadow-lg rounded-pill px-4" disabled={Object.keys(userAnswers).length !== quiz.questions.length}>
        ...
        {/* Unlock messages */}
        {score !== null && score >= MIN_SCORE && <p className="text-success text-center mt-3 fw-semibold">✅ Congratulations! You’ve unlocked the next topic.</p>}
        {score !== null && score < MIN_SCORE && <p className="text-danger text-center mt-3 fw-semibold">❌ You need at least {MIN_SCORE} correct answers to unlock the next topic.</p>}
